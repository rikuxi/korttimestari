// Omahan preflop-equity useampaa vastustajaa vastaan - hybridimenetelmä.
//
...
function parseArgs(argv) {
    const o = {
        players: 6, configs: 24, replicates: 8,
    ...
    if (o.players < 2 || o.players > 11) { console.error('--players 2..11'); process.exit(1); }
...
function main() {
    ...
    console.log(`Omaha, ${opts.players} pelaajaa (${opts.players - 1} vastustajaa)`);
    ...
    return runWorkers(opts, classOf, state).then(() => {
        ...
        const exactPath = path.resolve(__dirname, '..', 'data', 'preflop-omaha-2max-exact.json');
        ...
        const base = `preflop-omaha-${opts.players}max-hybrid`;
        const round = (x, n) => Number(x.toFixed(n));
        const output = {
            meta: {
                gameType: 'omaha', players: opts.players,
